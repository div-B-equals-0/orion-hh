...
+ Reproject all to the F502 corrected WCS
  #+begin_src python :return hdu656.header :results scalar
    ...
    hdu502 = fits.open(pcdir / "fov2f502.fits")[0]
    hdu502.header.update(**read_wcs(pcdir / "fov2f502-align-Robberto.wcs"))
    w502 = WCS(hdu502.header).celestial
    hdu502.header = w502.to_header().update(simple=True)
    hdu502.data = hdu502.data[0, :, :]

    hdu656 = fits.open(pcdir / "fov2f656.fits")[0]
    hdu656.header.update(**read_wcs(pcdir / "fov2f656-align-Robberto.wcs"))
    w656 = WCS(hdu656.header).celestial
    hdu656.header = w656.to_header().update(simple=True)
    hdu656.data = hdu656.data[0, :, :]
    newdata, _ = reproject_interp(hdu656, hdu502.header)
    hdu656.data = newdata

    hdu658 = fits.open(pcdir / "fov2f658.fits")[0]
    hdu658.header.update(**read_wcs(pcdir / "fov2f658-align-Robberto.wcs"))
    w658 = WCS(hdu658.header).celestial
    hdu658.header = w658.to_header().update(simple=True)
    hdu658.data = hdu658.data[0, :, :]
    newdata, _ = reproject_interp(hdu658, hdu502.header)
    hdu658.data = newdata

    hdu658.writeto(outdir / "hh204-pc-f658n.fits")
    hdu656.writeto(outdir / "hh204-pc-f656n.fits")
    hdu502.writeto(outdir / "hh204-pc-f502n.fits")
...
+ Reply to referee
  : We have made the suggested changes to Fig 1 in order to make the labelling clearer.  In addition, we have changed the font to make it more obvious that I, II, and III are roman numerals. 
